...
`makeConnectable()` giải quyết bài toán **timing**: đảm bảo tất cả subscriber đã subscribe xong trước khi publisher bắt đầu emit, Huy. Ba điểm cốt lõi:

**Publisher thường vs ConnectablePublisher:** Publisher thường emit ngay khi có subscriber đầu tiên → subscriber muộn có thể bỏ lỡ data. ConnectablePublisher thêm bước `connect()` — subscribe bao nhiêu subscriber tuỳ ý, rồi gọi `connect()` khi tất cả sẵn sàng → mọi subscriber đều nhận đầy đủ.

**Pattern chuẩn: `share()` + `makeConnectable()`** — `share()` đảm bảo chỉ 1 subscription vào upstream (1 network request thay vì N), `makeConnectable()` đảm bảo đợi tất cả subscriber sẵn sàng. Hai operator bổ trợ nhau cho bài toán "1 API call, nhiều consumer".

**`autoconnect()` là shortcut** — biến ConnectablePublisher thành publisher thường, tự connect khi subscriber đầu tiên subscribe. `Timer.publish` dùng pattern này phổ biến nhất: `Timer.publish(every: 1, on: .main, in: .common).autoconnect()`. Tiện nhưng mất khả năng kiểm soát timing — nếu cần kiểm soát, dùng `connect()` thủ công.

Sai lầm phổ biến nhất: **quên gọi `connect()`** → subscriber không bao giờ nhận value, không có lỗi hay warning — im lặng tuyệt đối, rất khó debug.
