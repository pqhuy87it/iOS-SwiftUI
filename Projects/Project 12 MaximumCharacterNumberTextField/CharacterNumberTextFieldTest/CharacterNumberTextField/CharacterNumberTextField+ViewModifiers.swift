...
public extension CharacterNumberTextField {
...
//    }
    ...
    func onClear(perform action: ((CustomTextField) -> Void)? = nil) -> Self {
        var view = self
        if let action = action {
    ...
    func onReturn(perform action: ((CustomTextField) -> Void)? = nil) -> Self {
        var view = self
        if let action = action {
