...
public struct PopView<Label, Destination, Tag>: View where Label: View, Destination: View, Tag: Hashable {
    @EnvironmentObject private var navViewModel: NavigationStack
    private let label: Label
    ...
    ///   - selection: A binding that triggers the navigation if and when its value matches the tag value.
    ///   - label: The actual view to tap to trigger the navigation.
    public init(destination: PopDestination = .previous, destinationView: Destination, tag: Tag, selection: Binding<Tag?>, @ViewBuilder label: () -> Label) {
		self.init(destination: destination, destinationView: destinationView, isActive: Binding.constant(false),
                  tag: tag, selection: selection, label: label)
    }

    private init(destination: PopDestination, destinationView: Destination, isActive: Binding<Bool>, tag: Tag?,
...
public extension PopView where Tag == Never {
    ...
    ///   - destination: The destination type of the pop operation.
    ///   - label: The actual view to tap to trigger the navigation.
    init(destination: PopDestination = .previous, destinationView: Destination, @ViewBuilder label: () -> Label) {
		self.init(destination: destination, destinationView: destinationView, isActive: Binding.constant(false),
                  tag: nil, selection: Binding.constant(nil), label: label)
    }
    ...
    ///   - isActive: A boolean binding that triggers the navigation if and when becomes true.
    ///   - label: The actual view to tap to trigger the navigation.
    init(destination: PopDestination = .previous, destinationView: Destination, isActive: Binding<Bool>, @ViewBuilder label: () -> Label) {
		self.init(destination: destination, destinationView: destinationView, isActive: isActive,
                  tag: nil, selection: Binding.constant(nil), label: label)
    }
}
